...
namespace Test
{
	TestTypesOfLights::TestTypesOfLights()
	{
		...
		auto planeShader = std::make_shared<Shader>("res/shaders/TypesOfLights.shader");
		...
		m_PlaneMat->SetUniform1i("u_LightType", m_LightType);
	...
	void TestTypesOfLights::OnRenderer()
	{
		...
		// Set custom uniform from outside whenever you want
		m_PlaneMat->SetUniform1i("u_LightType", m_LightType);
